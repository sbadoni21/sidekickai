...
import { parseLLMResponse, cleanLLMResponse } from "../utils/lmResponseParser"
...
// Improved Solution Section with Formatted Response
const ImprovedSolutionSection = ({
  title,
  content,
  isLoading
}: {
  ...
  <div className="space-y-2">
    ...
    {isLoading ? (
      ...
      <div className="text-xs text-gray-400">No solution available</div>
    )}
  </div>
)

// Legacy Solution Section (kept for compatibility)
...
const Solutions: React.FC<SolutionsProps> = ({ setView }) => {
  ...
  useEffect(() => {
    ...
    const cleanupFunctions = [
          ...
          mediaRecorder.onstop = async () => {
            ...
            reader.onloadend = async () => {
              ...
              try {
                ...
                // Prompt the LLM to answer the extracted question/text
                try {
                  const chatResponse = await window.electronAPI.invoke(
                    "gemini-chat",
                    result.text
                  )
                  const cleanedResponse = cleanLLMResponse(chatResponse)

                  // Store the LLM response as the current solution
                  ...
                  setSolutionData(cleanedResponse)
                } catch (chatErr) {
                  console.error(
                    "Error prompting LLM after audio analysis:",
                    chatErr
                  )
                }
              } catch (err) {
      ...
      // When the initial solution is generated, set the solution data
      window.electronAPI.onSolutionSuccess((data) => {
        ...
        // Clean the code response if it's a string
        const cleanedCode =
          typeof data.solution.code === "string"
            ? cleanLLMResponse(data.solution.code)
            : data.solution.code
  ...
  }, [isTooltipVisible, tooltipHeight, queryClient, refetch, setView])
  ...
  useEffect(() => {
    ...
    const unsubscribe = queryClient.getQueryCache().subscribe((event) => {
      if (event?.query.queryKey[0] === "problem_statement") {
        ...
        const audioResult = queryClient.getQueryData([
          "audio_result"
        ]) as AudioResult | undefined
        if (audioResult) {
              ...
              <div className="px-4 py-3 space-y-4 max-w-full">
                ...
                {problemStatementData?.validation_type === "manual" ? (
                  ...
                  <>
                    ...
                    {/* Solution Sections (with improved formatting) */}
                    {solutionData && (
                      <>
                        {/* Analysis Section */}
                        <ContentSection
                        ...
                        {/* Solution with Improved Formatting */}
                        <ImprovedSolutionSection
                          title={
                            problemStatementData?.output_format?.subtype ===
                            "voice"
                              ? "Response"
                        ...
                        {/* Complexity Section - Only for non-voice */}
                        {problemStatementData?.output_format?.subtype !==
                          "voice" && (
                          <ComplexitySection
